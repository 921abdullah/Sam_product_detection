...
class VerificationService:
    ...
    def local_crop_ransac_support_for_mask(
        self,
        ...
        candidate_crop_mask = candidate_mask[y1:y2, x1:x2].copy()
        candidate_crop_mask = dilate_binary_mask(
            candidate_crop_mask,
            dilation_px=config.local_ransac_mask_dilation_px,
        )

        if debug_name and debug_dir is not None:
            debug_path = Path(debug_dir)
            debug_path.mkdir(parents=True, exist_ok=True)
            save_rgb_as_bgr(str(debug_path / f"{debug_name}_before_crop.jpg"), before_crop_rgb)
            save_rgb_as_bgr(str(debug_path / f"{debug_name}_after_crop.jpg"), after_crop_rgb)
            cv2.imwrite(
                str(debug_path / f"{debug_name}_candidate_mask.jpg"),
        ...
        return {
            ...
            "reason": "supported" if has_local_support else "weak_local_support",
        }
